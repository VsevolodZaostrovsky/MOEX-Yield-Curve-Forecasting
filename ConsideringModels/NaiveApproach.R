...
dUseLT <- data.frame(year5 = exp(diff(log(df_train$Y5))) - 1,
                   year7 = exp(diff(log(df_train$Y7))) - 1,
                   year10 = exp(diff(log(df_train$Y10))) - 1,
...
rw12m <- arima(dUseST$month12, order=c(1, 0, 0))

rw5y <- arima(dUseLT$year5, order=c(1, 0, 0))
rw7y <- arima(dUseLT$year7, order=c(1, 0, 0))
rw10y <- arima(dUseLT$year10, order=c(1, 0, 0))
rw15y <- arima(dUseLT$year15, order=c(1, 0, 0))
...
pred_VAR <- predict(varST, n.ahead=296)
pred_rw12m <- predict(rw12m, n.ahead=296)
pred_auto_arima1y <- predict(auto_arima12m, n.ahead=296)

mae(pred_VAR$fcst$month12, df_test$Y1)
mae(pred_rw12m$pred, df_test$Y1)
mae(pred_auto_arima1y$pred, df_test$Y1)
